...
Buildモードのパネルに Network セクションがある:
- **Host world**: ポート5151でワールドを公開（ホストもプレイヤーとして参加）
- **Join**: アドレス（例 `192.168.x.x:5151`）を入れて参加。参加時にローカルの
  ワールドは置き換わるが `world.ron` は無傷。他のプレイヤーは青いカプセルで見える
- 家具の配置・移動・削除は全てサーバー権威で全員に同期。保存はホスト側のみ
- 制約: モデルファイル(.glb)は配布されない。参加者の models/ に同名ファイルが
  無い家具はプレースホルダー表示になる（モデル配布は今後の課題）
